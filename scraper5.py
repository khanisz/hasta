...
def pobierz_liste_lig(session):
    """Pobiera liste wszystkich lig/sezonow (liga_id + nazwa) z glownej strony."""
...
def pobierz_wszystkie_kolejki(session, liga_id, debug=False):
    ...
    for opt in opcje:
        ...
        tekst = opt.get_text(strip=True)  # np. "15 - 2026-09-20"
...
def parsuj_tabele_wynikow(html_fragment):
    ...
    W przeciwienstwie do poprzedniej wersji, NIE splaszcza tabeli do ciagu
    tekstu/cyfr (co powodowalo bledne przesuniecia wynikow miedzy meczami).
    Zamiast tego czyta bezposrednio strukture HTML: kazdy mecz to osobna
    komorka <th class="wyniki_mecz" data-liza_id1=".." data-liza_id2="..">
    zawierajaca wlasna, zagniezdzona tabelke z wynikiem setow i punktow.
    """
    soup = BeautifulSoup(html_fragment, "html.parser")
    # dokladnie tabele grup (np. EKSTRALIGA, 1 LIGA...) - nie zagniezdzone
    # tabelki z wynikami pojedynczych meczow (te maja inna klase)
    ...
    for tabela in tabele:
        ...
        if len(trs) < 3:
            continue  # potrzeba min. 2 wiersze naglowka + min. 1 wiersz danych
        ...
        # Gracze w kolumnach: kolejne <th> zawierajace zagniezdzone <div>
        # (nazwisko + imie). Gdy natrafimy na naglowek bez divow (miejsce,
        # wygrane mecze, sety, punkty...), lista graczy sie konczy.
        col_players = []
        ...
        data_rows = trs[2:2 + n]  # dokladnie tyle wierszy danych ilu graczy
        ...
        for r, row in enumerate(data_rows):
            ...
            for gracz_2, cell in zip(oczekiwani_przeciwnicy, mecz_cells):
                ...
                # Klucz unikalnosci: wolimy ID zawodnikow z bazy (pewniejsze
                # niz nazwiska, ktore moga sie powtarzac), z fallbackiem na
                # nazwiska gdy z jakiegos powodu ID brakuje.
...
def parsuj_wszystkie_ligi(output_file="wyniki_wszystkich_lig.json", debug=False, limit=None):
    session = requests.Session()

    print("Pobieranie listy wszystkich lig/sezonow...")
    ligi = pobierz_liste_lig(session)
    print(f"Znaleziono {len(ligi)} lig/sezonow.")
    ...
    for idx, liga in enumerate(ligi):
        ...
        # ta sama para gra ze soba w kilku roznych kolejkach (np. drabinka,
        # rewanz), to sa to ROZNE, prawdziwe mecze, wiec nie deduplikujemy
        # miedzy kolejkami. Deduplikacja wewnatrz jednej kolejki (symetryczne
        # powielenie tego samego meczu w macierzy wiersz/kolumna) nadal
        # dzieje sie w parsuj_tabele_wynikow.
        mecze_z_ligi = []
...
if __name__ == "__main__":
    ...
    # na jednej lidze, przez wszystkie jej kolejki, czy deduplikacja dziala
    # poprawnie (w konsoli zobaczysz per-kolejke ile bylo meczow w tabeli
    # i ile z nich bylo NOWYCH - to pokaze, czy tabela faktycznie jest
    # kumulatywna, tak jak zakladamy).
    # parsuj_wszystkie_ligi(output_file="wyniki_test.json", debug=True, limit=1)

    # UWAGA: pelny przebieg (KROK 2 ponizej) robi zapytanie per KOLEJKA per
    # LIGA, wiec przy ~120 ligach i ~10-15 kolejkach kazda to nawet >1000
    # zapytan - liczy sie na minuty, nie sekundy.

    # KROK 2: gdy krok 1 zadziala poprawnie, odkomentuj ponizsze, zeby pobrac
    # WSZYSTKIE ligi/sezony na raz:
    parsuj_wszystkie_ligi(output_file="wyniki_wszystkich_lig.json", debug=False)